...
| `docs/modes.md` | The mode taxonomy — Campaign/Skirmish/PvP as three front doors over one match engine; skirmish setup flow; PvP army-select/map-pools/ranking |
...
Asset creation here is **scripted, not hand-made**: every visible model / sound / icon is
generated by a headless tool the same way D41 scripts Blender — commit the *generator
script* + a manifest entry (`source` / `license` / `sha256`), never an opaque binary blob.
When a task needs an asset, **reach for these first** instead of requesting commissioned
art. All are installed machine-wide and on PATH:

- **Blender** (`bpy`, `--background --python`) — procedural/greybox meshes, terrain, rig/anim, glTF export
- **gltfpack** — glTF mesh/texture compression for the mobile / 200-unit budget
...
pal crate stays dependency-free — the shipped CP-6 cue set, D100).

Full rationale + the can/can't boundary: [`docs/content-pipeline.md`](docs/content-pipeline.md)
§6 (**D46** logs the toolchain; **D41** the Blender method; **D74** the font atlas; **D100** the
