...
| [`docs/modes.md`](docs/modes.md) | Game modes — Campaign / Skirmish / PvP as three distinct front doors over one match engine; the skirmish map/army/difficulty setup flow; the PvP meta (per-queue **army** policy — ranked randomizes with a 1v1 anti-mirror guard, [D130](docs/decisions.md) — + per-queue map pools + symmetry gate, matchmaking/ranking + determinism-backed result verification; rating model open as [Q29](docs/open-questions.md); the specced cooperative **team mode** §4d, [D131](docs/decisions.md)) |
...
| [`docs/embodied-feel-floor.md`](docs/embodied-feel-floor.md) | The embodied game-feel floor — the written CP-2 acceptance checklist a playtester reads to judge "does the gunplay clear the bar?"; presentation/feel only, never sim state |
| [`docs/factions.md`](docs/factions.md) | Factions — real-army asymmetry, **US Army vs French Army** ([D68](docs/decisions.md)/[D71](docs/decisions.md)); the fairness-bounded roster model layered over `UnitKind`; WS-A–E built, incl. the native army-select screen on both platforms (desktop egui + Android Compose `ArmySelectScreen.kt`) |
...
| [`docs/plans/pve-campaign-plan.md`](docs/plans/pve-campaign-plan.md) | PvE campaign plan (IN PROGRESS — D58–D61) — the first shippable product: WS-A (objectives/seize mission), WS-D (HUD layout editor), WS-E (difficulty/modifiers) landed; WS-C (gunsmith) done on both platforms; WS-B (Operations hub) functionally complete on both platforms incl. the gated-node Compose launch wire — a 15-node, five-conflict atlas-grouped graph ships (D105 four modern; D121 adds Normandy '44, the first historical conflict); more mission content remains (D32's native-shell approach is settled) |
...
| [`docs/plans/factions-plan.md`](docs/plans/factions-plan.md) | Factions plan (IN PROGRESS — [D68](docs/decisions.md)/[D71](docs/decisions.md)) — **US Army vs French Army**: WS-A/B/C/E all built; WS-D seam + PvE scenario seeding + army-tilted troops built; the native army-select screen has landed on both platforms (desktop egui + Android Compose `ArmySelectScreen.kt`) |
| [`docs/plans/team-mode-plan.md`](docs/plans/team-mode-plan.md) | Team-mode plan (DESIGN PASS — [D131](docs/decisions.md)) — cooperative **multi-commander** PvP (target 5v5, ramped 2v2→3v3→5v5): *every* player does both jobs (no dedicated commander/soldiers — the anti-Eximius stance), `N` sub-forces + sectors + one win condition, shared team vision but **never** shared command; the pick/ban maps to army ([D130](docs/decisions.md)) + a future doctrine layer, not heroes; open forks [Q31](docs/open-questions.md)/[Q32](docs/open-questions.md) |
