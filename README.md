...
| [`docs/plans/compose-shell-parity.md`](docs/plans/compose-shell-parity.md) | Compose shell parity plan (PLANNED — [D78](docs/decisions.md)/[D79](docs/decisions.md)) — bring Android's out-of-match Compose shell up to the desktop egui shell: the linchpin Compose→`NativeActivity` launch-config seam (Tier 0), then Settings/Profile/About + title mode-split (Tier 1), then gunsmith + campaign mission-select (Tier 2); PvP/lobby/store/consent stay blocked |
