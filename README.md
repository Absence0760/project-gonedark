...
| `Cargo.toml` + `core/ pal/ render/ engine/ pal-desktop/ pal-android/ app/ sim-runner/ net-sim-runner/ viz-runner/ server/` | **The Rust engine workspace.** `core` = deterministic fixed-point sim (zero platform deps) — a real flow field, the Phase 2 game-systems modules (`combat`/`terrain` cover+LoS, `territory`, `economy`/camps, `fog`, `orders`, `alerts`, `event`), the sans-I/O `lockstep` command-exchange loop + wire codec ([D27](docs/decisions.md)), and the authoritative-snapshot `persist` serializer (`Sim::serialize/deserialize` sharing the checksum field-walk, terrain by map-id, for reconnect/resume — [D28](docs/decisions.md)), all fixed-point and checksum-folded ([D23](docs/decisions.md)); `pal` = platform traits (incl. the touch-UI input intents, the `Audio` mix seam, [D24](docs/decisions.md), and the `Transport` opaque-byte-frame netcode seam, [D27](docs/decisions.md)); `render` = real `wgpu` instanced renderer with fog-of-war filtering, the embodied directional alert HUD (`fog`/`hud`, [D24](docs/decisions.md)), and a white selection rim ([D26](docs/decisions.md)); `engine` = the platform-agnostic game loop (sim+render+fixed-tick+cameras+command/embodiment) that both hosts drive ([D20](docs/decisions.md)), also driving the embodied audio mix, unit selection, and the order/stance command vocabulary (`audio`/`selection`/`command_ui`, [D24](docs/decisions.md)), with the fixed-tick loop now sourcing each tick's command set through `core::lockstep` (single-player = a 1-peer delay-0 session, [D27](docs/decisions.md)); `pal-desktop` = real `winit`+`wgpu` backend with optional `cpal` audio output (opt-in `audio` feature → `pnpm play:audio`, [D26](docs/decisions.md)) plus an in-process `LoopbackTransport` for dev/test ([D27](docs/decisions.md)); `app` = thin winit desktop host; `pal-android` = JNI/cargo-ndk backend whose `android_main` drives the same `engine` loop (builds an arm64 APK); `sim-runner` = headless single-client checksum driver; `net-sim-runner` = headless **2-peer lockstep** checksum driver — runs both `core::lockstep` peers in-process over a seeded channel, asserts they agree + match a no-network reference, emits the agreed stream for the ADD-ONLY `compare-net` CI job (`pnpm desktop:sim:net`, [D27](docs/decisions.md)); `viz-runner` = headless **offscreen render** smoke test (renders `Game` to a texture, reads pixels back, asserts the command view draws + embodiment goes dark + the alert HUD draws, and writes PNGs — `pnpm desktop:viz`, needs a GPU so it's local-only, not CI); `server` = backend placeholder. See [`docs/phase-1-plan.md`](docs/phase-1-plan.md) |
