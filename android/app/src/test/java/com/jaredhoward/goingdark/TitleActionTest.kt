...
class TitleActionTest {
    ...
    @Test
    fun pve_opens_the_loadout_gunsmith() {
        assertEquals(TitleRoute.Loadout, resolveTitleAction(TitleAction.Pve))
    }

    @Test
    fun pvp_opens_the_loadout_gunsmith() {
        assertEquals(TitleRoute.Loadout, resolveTitleAction(TitleAction.Pvp))
    }
    ...
    @Test
    fun every_action_routes_to_a_distinct_destination_except_the_shared_loadout() {
        // Exhaustive sweep: every action maps, and only Pve/Pvp share a destination (the gunsmith),
        // matching the Rust seam where those two fold to OpenLoadout and the rest are 1:1.
        val routes = TitleAction.entries.associateWith { resolveTitleAction(it) }
        assertEquals(TitleRoute.Loadout, routes[TitleAction.Pve])
        assertEquals(TitleRoute.Loadout, routes[TitleAction.Pvp])
