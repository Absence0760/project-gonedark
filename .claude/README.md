...
| [`viz-green-guard.py`](hooks/viz-green-guard.py) | PreToolUse on `Bash` | **Project-specific — keeps the `viz-runner` visual baseline green.** CLAUDE.md requires every visual workstream to land with viz green, but viz is a GPU-gated LOCAL tool (not in the no-GPU CI matrix), so render commits (D52 silhouettes, WS-D footprints, palette threading) had been landing RED on `main` with nothing to catch it. When a `git commit` stages a path under `render/` or `viz-runner/`, this runs `cargo run -p gonedark-viz-runner` and **denies** the commit only on a confirmed visual-assertion failure — naming the failed assertion(s) and pointing at `pnpm desktop:viz` to reproduce. Commits that touch no render path skip it (viz never runs). **Fails OPEN** on everything else: no GPU adapter (`SKIP: no wgpu adapter`), a build error it can't attribute to viz, a timeout, or any internal hook error → allow (a one-line stderr warning for the GPU/build cases). Only a real RED baseline blocks. Covered by [`viz-green-guard.test.py`](hooks/viz-green-guard.test.py) (green here). |
