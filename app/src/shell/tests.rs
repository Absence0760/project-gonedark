...
fn stamp_normalises_case_and_trims_whitespace() {
    assert_eq!(
        build_stamp("  RELEASE ", " 1.2.3 "),
        "build release · v1.2.3"
    );
}
...
fn quit_exits() {
    assert_eq!(
        resolve_title_action(TitleAction::Quit),
        HostTransition::Exit
    );
}
...
fn stamp_matches_the_path_main_uses() {
    ...
    assert_eq!(
        build_stamp(build_channel(true), "0.0.0"),
        "build dev · v0.0.0"
    );
    assert_eq!(
...
fn done_is_a_screen_transition_that_leaves_the_editor_alone() {
    ...
    assert_eq!(
        apply_loadout_action(LoadoutAction::Done, &mut ed),
        LoadoutStep::Done
    );
...
fn rgb8_rounds_srgb_to_8bit() {
    ...
    assert_eq!(
        rgb8(gonedark_render::theme::INK),
        egui::Color32::from_rgb(0x07, 0x09, 0x0C)
    );
}
...
fn shell_in_match_variants_are_pinned_and_distinct() {
    ...
    assert_ne!(
        PANEL,
        rgb8(theme::PANEL),
        "shell PANEL is lighter than the in-match HUD PANEL"
    );
    assert_ne!(
        AMBER,
        rgb8(theme::AMBER),
        "shell AMBER is cooler than the in-match HUD AMBER"
    );
}
...
fn shell_style_type_scale_matches_the_named_ramp_and_descends() {
    ...
    // The hierarchy is strictly descending (a guard against a future edit inverting two
    // sizes). `const` asserts: the compared values are consts, so the guard fires at
    // compile time on any build, not just when this test runs.
    const _: () = assert!(TYPE_DISPLAY > TYPE_HEADING);
    const _: () = assert!(
        TYPE_HEADING > TYPE_STAT,
        "a stat numeral stays below the screen banner"
    );
    const _: () = assert!(
        TYPE_STAT > TYPE_SUBHEAD,
        "a stat numeral reads as a figure, not body text"
    );
    const _: () = assert!(TYPE_HEADING > TYPE_SUBHEAD);
    const _: () = assert!(TYPE_SUBHEAD >= TYPE_BUTTON);
    const _: () = assert!(TYPE_BUTTON > TYPE_BODY);
...
fn shell_style_makes_controls_legible_on_the_dark_card() {
    ...
    assert!(
        ...
        "scrollbar must be solid, not hover-only"
    );
}
...
fn over_backdrop_top_settles_at_the_optical_centre_and_clamps() {
    ...
    assert!(
        (top - 252.0).abs() < 0.01,
        "expected (1000-400)*0.42, got {top}"
    );
...
fn per_option_delta_text_changes_with_the_selection() {
    ...
    assert_eq!(
        stat_delta_summary(&ed.option_delta(LoadoutSlot::Grip)),
        "no change"
    );
...
fn accessibility_toggles_round_trip_and_default_when_missing() {
    ...
    for (cvd, snd) in [(true, false), (false, true), (true, true), (false, false)] {
        ...
        assert_eq!(
            s2.visual_sound_cues, snd,
            "sound toggle survives round-trip"
        );
    }
...
fn alert_cue_mode_round_trips_every_mode_and_defaults_when_missing() {
    ...
    for &mode in &AlertCueMode::ALL {
        ...
        let (s2, _, _, _) = decode_shell_prefs(&blob);
        assert_eq!(
            s2.alert_cue_mode, mode,
            "{mode:?} alert-cue mode survives round-trip"
        );
    }
    // A blob missing the key (an older save) decodes to Off; a garbage ordinal also falls back.
    let (s, _, _, _) = decode_shell_prefs("gonedark-shell 1\nmaster=0.5\n");
    assert_eq!(
        s.alert_cue_mode,
        AlertCueMode::Off,
        "missing alertcue → Off"
    );
    let (s2, _, _, _) = decode_shell_prefs("alertcue=999\n");
    assert_eq!(
        s2.alert_cue_mode,
        AlertCueMode::Off,
        "out-of-range alertcue → Off"
    );
}
...
fn settings_clamp_rebounds_every_out_of_range_field() {
    ...
    assert_eq!(
        s.fov_deg,
        SettingsState::FOV_MAX,
        "over-range FOV clamps to the ceiling"
    );
...
fn confirm_gate_requires_two_clicks_to_fire() {
    ...
    assert!(
        !fired_first,
        "first click never fires the destructive action"
    );
...
fn egui_key_maps_to_keyid_at_the_boundary() {
    ...
    assert_eq!(
        egui_key_to_keyid(egui::Key::Backtick),
        Some(KeyId::Backquote)
    );
...
fn keybinds_survive_the_shell_prefs_round_trip_and_default_when_missing() {
    ...
    assert_eq!(
        s.keybinds.rebind(GameAction::Jump, KeyId::V),
        RebindOutcome::Bound
    );
    assert_eq!(
        s.keybinds.rebind(GameAction::Embody, KeyId::T),
        RebindOutcome::Bound
    );
    let blob = encode_shell_prefs(
        &s,
...
fn quality_cycles_through_all_choices_and_wraps() {
    ...
    for choice in QualityChoice::ALL {
        assert!(
            ...
            "{choice:?} must appear in the cycle"
        );
    }
...
fn sample_state() -> (SettingsState, ProfileState, LoadoutEditor, ArmySelectState) {
    ...
    let army = ArmySelectState { selected: Army::Fr };
...
fn empty_or_garbage_blob_decodes_to_defaults() {
    for blob in [
        ...
        assert_eq!(
            s,
            SettingsState::default(),
            "blob {blob:?} → default settings"
        );
        assert_eq!(
            p,
            ProfileState::default(),
            "blob {blob:?} → default profile"
        );
        assert_eq!(
            l.current(),
...
fn decode_tolerates_out_of_range_and_unparseable_values() {
    ...
    assert_eq!(
        s.mouse_sensitivity,
        SettingsState::SENS_MAX,
        "over-range sens clamps"
    );
    assert!(
        !s.invert_look_y,
        "unparseable bool keeps the default (false)"
    );
    assert_eq!(
        s.quality,
        QualityChoice::Auto,
        "out-of-range quality ordinal → default"
    );
    assert_eq!(
        p.faction,
        FactionPref::UsArmy,
        "out-of-range faction ordinal → default"
    );
    ...
    assert_eq!(
        l.current(),
        Loadout::STANDARD,
        "out-of-range slot ordinals → Standard"
    );
    assert_eq!(
        a,
        ArmySelectState::default(),
        "out-of-range army ordinal → default (US)"
    );
}
...
fn decode_sanitizes_and_strips_a_newline_injected_callsign() {
    ...
    assert_eq!(
        blob.lines().filter(|l| l.starts_with("callsign=")).count(),
        1
    );
...
fn army_choose_edits_the_selection_and_stays() {
    ...
    assert_eq!(
        step,
        ArmySelectStep::Stay,
        "a choice keeps the player on-screen"
    );
...
fn army_labels_and_flavor_are_distinct_ascii() {
    ...
    for text in labels.iter().chain(flavors.iter()) {
        assert!(
            ...
            "{text:?} must be non-empty ASCII"
        );
    }
    assert_ne!(labels[0], labels[1], "the two armies have distinct names");
    assert_ne!(
        flavors[0], flavors[1],
        "the two armies have distinct flavour"
    );
...
fn decode_army_rejects_neutral_and_missing_falling_back_to_default() {
    ...
    assert_eq!(
        decode_army(Some(&"3")),
        default,
        "UsWw2 ordinal → default (not modern-selectable)"
    );
    assert_eq!(
        decode_army(Some(&"4")),
        default,
        "Germany ordinal → default (not modern-selectable)"
    );
}
...
fn field_manual_blurb_is_the_canonical_three_sentence_copy() {
    ...
    assert!(
        FIELD_MANUAL_BLURB.is_ascii(),
        "the blurb must render in egui's default font"
    );
...
fn controls_reference_leads_with_the_going_dark_concept_section() {
    ...
    let rows = controls_reference();
    assert_eq!(
        rows[0].group, "GOING DARK",
        "the concept section must lead the manual"
    );
    ...
    assert!(
        last_concept < first_keymap,
        "the concept section is not interleaved with the keymap"
    );
}
...
fn cleared_tiles_stay_playable_for_replay() {
    ...
    assert_eq!(
        playable_node(&entries[0]),
        Some(NodeId(0)),
        "a cleared node replays"
    );
...
fn next_operation_prefers_fresh_progress_over_a_higher_index_replay() {
    ...
    assert_eq!(
        op.node,
        NodeId(0),
        "fresh progress beats a higher-index replay"
    );
...
fn hub_sections_order_conflicts_then_operations_then_trailing_ungrouped() {
    ...
    assert_eq!(
        sections[0].operation.map(|(id, _)| id),
        Some(OperationId(0))
    );
    ...
    assert_eq!(
        sections[1].conflict, None,
        "a conflict header draws once, on its first section"
    );
    assert_eq!(
        sections[1].operation.map(|(id, _)| id),
        Some(OperationId(1))
    );
    ...
    assert_eq!(
        sections[2].operation, None,
        "ungrouped nodes render in an untitled section"
    );
...
fn the_globe_focuses_the_conflict_being_fought_and_pins_every_conflict() {
    ...
    assert_eq!(
        pins.iter().filter(|p| p.focused).count(),
        1,
        "exactly one focus"
    );
...
fn the_atlas_opens_settled_on_the_fought_conflict_at_its_opening_year() {
    ...
    assert_eq!(
        state.year, 2027,
        "opens scrubbed to the fought conflict's first year"
    );
...
fn atlas_navigation_clamps_but_never_blocks() {
    ...
    assert_eq!(
        state.view.zoom,
        gonedark_render::globe_backdrop::GlobeView::ZOOM_MAX
    );
    apply_atlas_action(AtlasAction::Zoom(-1_000.0), &mut state, &campaign);
    assert_eq!(
        state.view.zoom,
        gonedark_render::globe_backdrop::GlobeView::ZOOM_MIN
    );
...
fn clicking_the_focused_pin_picks_it_and_empty_ocean_picks_nothing() {
    ...
    assert_eq!(
        pick_conflict(&campaign, &state, aspect, p),
        Some(state.selected)
    );
...
fn a_continue_deep_link_resolves_its_node_to_the_right_conflict() {
    ...
    for node in 0..campaign.len() as u32 {
        ...
        if let Some(i) = conflict_index_of(&campaign, node) {
            ...
            assert_eq!(
                campaign.operation(op).unwrap().conflict,
                campaign.conflicts()[i].id
            );
        } else {
...
fn the_hub_filtered_to_a_conflict_shows_only_its_operations() {
    ...
    for s in &only {
        let (op, _) = s
            .operation
            .expect("filtered sections are always operation-backed");
...
fn the_shipped_wars_frame_their_battlefields_on_screen() {
    ...
    for conflict in campaign.conflicts() {
        let view =
            overview_view(&campaign, conflict.id).expect("every shipped war has anchored battles");
        ...
        for op in campaign.operations_in(conflict.id) {
            for n in campaign.nodes_in(op) {
                ...
                let p = project_pin(view, 16.0 / 9.0, lat, lon).unwrap_or_else(|| {
                    panic!(
                        "{}'s battle {n:?} is not visible in its overview",
                        conflict.name
                    )
                });
                assert!(
...
fn battle_pins_tone_and_focus_follow_progress() {
    ...
    let pins = battlefield_pins(
        &campaign,
        ConflictId(0),
        next_battle_in(&campaign, ConflictId(0)),
    );
    ...
    assert!(
        pins.iter()
            .all(|p| p.scale == OVERVIEW_PIN_SCALE && p.scale > 1.0),
...
fn picking_a_battle_honours_the_playable_gate() {
    ...
    assert_eq!(
        pick_battle(&campaign, ConflictId(0), view, aspect, alpha),
        Some(NodeId(0))
    );
    // Bravo is Locked — a dead-center click on it picks nothing (never launches).
    assert_eq!(
        pick_battle(&campaign, ConflictId(0), view, aspect, bravo),
        None
    );
    // Empty ground picks nothing.
    assert_eq!(
        pick_battle(&campaign, ConflictId(0), view, aspect, [0.95, 0.95]),
        None
    );
...
fn the_atlas_drag_follows_the_surface_across_latitudes() {
    ...
    assert!(
        (eq_dyaw - 100.0 * DRAG_SENS / eq.view.zoom).abs() < 1e-5,
        "no equatorial regression"
    );
    ...
    assert!(
        ((mid.view.yaw - y0) / eq_dyaw - 2.0).abs() < 1e-3,
        "60N drags ~2x the lon"
    );
        ...
        for look in [
            ...
            for op in shipped.operations_in(conflict.id) {
                for n in shipped.nodes_in(op) {
                    let Some((lat, lon)) = shipped.node(n).unwrap().anchor else {
                        continue;
                    };
                    let p = project_pin(view, 16.0 / 9.0, lat as f32 / 10.0, lon as f32 / 10.0)
                        .unwrap_or_else(|| {
...
fn site_waypoints_walk_the_chain_and_keep_locked_ground_visible() {
    ...
    assert_eq!(
        (sites[0].node, sites[0].order, sites[0].title.as_str()),
        (NodeId(0), 1, "Alpha")
    );
    assert_eq!(
        (sites[1].node, sites[1].order, sites[1].title.as_str()),
        (NodeId(1), 2, "Bravo")
    );
    ...
    assert!(
        sites[1].ndc.is_some(),
        "a locked site still gets a screen position"
    );
    ...
    assert_eq!(
        (legs[0].0, legs[0].1),
        (sites[0].ndc.unwrap(), sites[1].ndc.unwrap())
    );
    ...
    assert_eq!(
        site_color(NodeProgress::Available),
        egui::Color32::from_rgb(245, 158, 51)
    );
    assert_eq!(
        site_color(NodeProgress::Locked),
        egui::Color32::from_rgb(107, 120, 148)
    );
    assert_eq!(
        site_color(NodeProgress::Cleared {
    ...
    assert!(
        label_side_for(&zigzag, 0),
        "the rightmost site keeps its right label"
    );
    assert!(
        !label_side_for(&zigzag, 2),
        "a crowded right lane flips the label left"
    );
...
fn the_hub_backdrop_view_flies_then_lands_on_the_target() {
    ...
    assert!(
        mid != from && mid != to,
        "mid-flight is between the endpoints"
    );
...
fn the_atlas_return_flight_lands_on_the_opened_view_unless_the_player_grabs_it() {
    ...
    assert_eq!(
        state.view, hub_view,
        "no cut: the first frame is the hub's camera"
    );
    ...
    assert_eq!(
        ...
        "landed exactly on the plain opened view"
    );
    ...
    assert!(
        grabbed.flight.is_none(),
        "a drag hands the camera to the player"
    );
    // ...and so does a zoom.
    let mut zoomed = AtlasState::opened_from(&campaign, hub_view);
    apply_atlas_action(AtlasAction::Zoom(1.0), &mut zoomed, &campaign);
    assert!(
        zoomed.flight.is_none(),
        "a zoom hands the camera to the player"
    );
    ...
    assert!(
        scrubbed.flight.is_some(),
        "scrub/select never steal the camera"
    );
}
...
fn difficulty_labels_are_distinct_ascii() {
    let labels: Vec<&str> = Difficulty::ALL
        .iter()
    ...
    assert_eq!(
        sorted.len(),
        labels.len(),
        "every tier has a distinct label"
    );
}
...
fn briefing_cycle_advances_the_selection_and_stays() {
    ...
    assert_eq!(
        selected,
        Difficulty::Regular,
        "cycle advances the live selection"
    );
}
...
fn skirmish_default_is_the_neutral_shipped_match() {
    ...
    assert_ne!(
        cfg.player_army, cfg.enemy_army,
        "the default reads as a two-army fight"
    );
...
fn every_battlefield_resolves_to_a_real_deploy() {
    ...
    assert!(BATTLEFIELDS
        .iter()
        .any(|b| matches!(b.kind, BattlefieldKind::LibraryMap(_))));
}
...
fn skirmish_config_edits_apply_in_place_and_stay() {
    ...
    assert_eq!(
        state.enemy_army,
        Army::Fr,
        "cycling the player side never edits the enemy"
    );
    assert_eq!(
...
fn sketch_centre_cell_starts_at_the_panel_midpoint() {
    ...
    let panel = egui::Rect::from_min_max(egui::pos2(0.0, 0.0), egui::pos2(256.0, 256.0));
    assert_eq!(
        cell_sketch_rect(panel, 64, 64).min,
        egui::pos2(128.0, 128.0)
    );
}
...
fn zone_outlines_wear_the_faction_hues() {
    ...
    assert_eq!(
        zone_outline_color("player"),
        rgb8(gonedark_render::theme::PLAYER)
    );
    assert_eq!(
        zone_outline_color("enemy"),
        rgb8(gonedark_render::theme::ENEMY)
    );
...
fn reseed_player_army_follows_the_identity_pick_and_bumps_a_colliding_enemy() {
    ...
    assert_eq!(
        state.enemy_army,
        Army::Us,
        "a non-colliding enemy pick is preserved"
    );
...
fn pvp_queue_table_is_distinct_ascii_and_complete() {
    ...
    for q in PVP_QUEUES {
        ...
        assert!(
            !q.id.is_empty() && !q.name.is_empty() && !q.blurb.is_empty() && !q.status.is_empty()
        );
    }
