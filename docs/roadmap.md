...
| **Match setup** | Army/loadout composition, map + mode select; skirmish-vs-PvP entry. **Designed:** [`modes.md`](modes.md) (the mode taxonomy + the skirmish map/army/difficulty setup flow). **Landed, both shells:** the three distinct doors ([D101](decisions.md)) — skirmish match-setup (battlefield/armies/tier) + the honest PvP staging door ([`plans/compose-shell-parity.md`](plans/compose-shell-parity.md) §12 items 6–7 closed) — and the map-library picker ([D102](decisions.md): the `BATTLEFIELDS` table, both shells) with its MapSpec-derived map card ([D109](decisions.md): sketch + integer metrics, both shells); the baker's lint-PNG/manifest metrics + baked/generated maps (behind the D77 loader) are the remaining target-model half | order/stance vocab (D25) |
...
- [x] Match-setup / skirmish-entry screen (Phase 4) — designed in [`modes.md`](modes.md) §3; step 1
  of its build order (no net dependency; all seams landed). **Landed on both shells** — the
  skirmish-setup screen behind the title's SKIRMISH door (battlefield / both armies / opponent
  tier, wired through the landed pre-tick launch seams; the Compose twin is
  [`compose-shell-parity.md`](plans/compose-shell-parity.md) §12 item 6) **and the map-library
  picker** ([D102](decisions.md): the `BATTLEFIELDS` table lists the embedded authored maps beside
  the standing battles, booted via `seed_positioned_skirmish`; the MapSpec-derived map card
  landed next, [D109](decisions.md)). Target-model remainder, not a blocker: the baker's
