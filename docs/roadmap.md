...
| **Match setup** | Army/loadout composition, map + mode select; skirmish-vs-PvP entry. **Designed:** [`modes.md`](modes.md) (the mode taxonomy + the skirmish map/army/difficulty setup flow). **Desktop landed:** the three distinct doors ([D101](decisions.md)) — skirmish match-setup (battlefield/armies/tier) + the honest PvP staging door; Android twins owed ([`plans/compose-shell-parity.md`](plans/compose-shell-parity.md) §12 items 6–7) | order/stance vocab (D25) |
