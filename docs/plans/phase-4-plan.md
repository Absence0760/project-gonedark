...
| 2 | **Onboarding / tutorial** | Teach the going-dark cost; telegraph the blindness *before* it bites; a guided first-possession beat. The single most important screen — invariant #6 lives or dies on whether a loss reads as *"I stayed too long."* | **[Q5](../open-questions.md)** (PvE is the natural teach surface); invariant #6 | **LANDED (functionally) — [D58](../decisions.md) picked PvE-first, and CP-7 ([`roadmap.md`](../roadmap.md)) shipped the teach surface:** `engine::onboarding`'s 3-beat WentDark→Lingering→StayedTooLong machine is wired into PvE mission 1 on both platforms. **Owed:** the by-eye/by-ear playtest confirming a loss actually reads as *"I stayed too long"* (invariant #6). |
| 3 | **Settings** | Graphics tiers (↔ device quality tiers, §4 WS-C), audio-mix levels, the touch-layout/rebind editor (configures the D14 scheme), desktop key/gamepad rebinds, **accessibility** | invariant #6 (accessibility) | **PARTIAL (desktop) — [D75](../decisions.md).** The desktop Settings screen has landed in the egui shell with **audio master/SFX volume + look sensitivity/invert-Y wired through to the host** (`pal`/`pal-desktop`), plus fullscreen and a (dormant) quality choice; a Profile + About/field-manual screen landed alongside. Still **BLOCKED for the rest**: the **accessibility cues** (§5) the going-dark channel's fairness depends on, the touch-layout/rebind editor, and key/gamepad rebinds — so it stays native UI on the seam (D32) and can't be a thin afterthought. The Android Compose Settings surface (audio/look subset) has since landed at parity too — see [`compose-shell-parity.md`](compose-shell-parity.md) §12. |
| 4 | **Match setup** | Army/loadout composition, map + mode select; skirmish-vs-PvP entry | order/stance vocab (D25); **[Q5](../open-questions.md)** | **LANDED — the skirmish half is complete on both shells** ([`modes.md`](../modes.md) §3 / build-order step 1): the skirmish-setup screen behind the title's SKIRMISH door (battlefield / both armies / opponent tier; the Compose twin is [`compose-shell-parity.md`](compose-shell-parity.md) §12 item 6) **plus the map-library picker** ([D102](../decisions.md), plus its [D109](../decisions.md) MapSpec-derived map card — target-model remainder: the baker's lint-PNG/manifest metrics, baked/generated maps after the D77 loader). The **PvP half stays BLOCKED — [Q5](../open-questions.md)** *and* Phase 3 net (modes.md §5 steps 2–4); PvP's title door is the honest [D101](../decisions.md) staging screen until then. |
| 5 | **Lobby & matchmaking** (PvP) | Party/invite, connection-quality readout, ready-up. *Seam:* the net plumbing is Phase 3; only the surface is Phase 4 | **Phase 3 netcode** (D27 lockstep, host-RTT, relay/matchmaking); **[Q5](../open-questions.md)** | **BLOCKED — Phase 3 netcode + [Q5](../open-questions.md).** The connection-quality readout and ready-up sit on the live RTT *sample source* (transport ping/pong — the one stub the landed `engine::net_tuning` estimator seam still needs) + relay/matchmaking that Phase 3 still owes; no PvP target until Q5. Custom lobby is [`modes.md`](../modes.md) §5 step 2, quick match/ranked steps 3–4. |
...
> **Android Compose parity:** ✅ substantially complete. The desktop egui shell had grown several
> surfaces (Settings/Profile/About [D75](../decisions.md), gunsmith, campaign mission-select) that the
> Android Compose shell lacked; all three tiers (the Compose→`NativeActivity` launch-config seam, then
> Settings/Profile/About + title mode-split, then gunsmith + campaign) plus a parity-gap sweep have
> since landed — full status, and the structural items still open, in
> [`compose-shell-parity.md`](compose-shell-parity.md) §12 ([D78](../decisions.md)/[D79](../decisions.md)).

| Surface | Blocker (beyond the WS-A seam + missing native project) |
|---|---|
| Boot & title | **LANDED (Android [D35](../decisions.md) + desktop [D36](../decisions.md))** (Compose title/landing screen on Android; egui title screen on desktop); the Android Compose shell has since reached parity with desktop ([`compose-shell-parity.md`](compose-shell-parity.md) §12); only the **iOS** native shell still pending (no iOS target) |
| Onboarding / tutorial | **LANDED (functionally)** — [D58](../decisions.md) resolved Q5; the CP-7 teach machine ships in PvE mission 1 on both platforms ([`roadmap.md`](../roadmap.md)); owed: the invariant-#6 playtest |
...
- **[D58](../decisions.md) — Q5 resolved: PvE-first** ✅ **RECORDED** — the teach surface and
  ...
  lobby (5) stay blocked, now on **Phase 3 netcode** rather than Q5. (Onboarding's §2/§4 cells now
  record CP-7's landed teach machine; where a cell still cites Q5 — the PvP halves of match setup
  and the lobby — treat that as superseded by D58: PvE halves are unblocked, PvP halves wait on
  net.)
...
still lacks and the Q5/Q9/Q11/Phase-3/backend blockers in §2 — onboarding, lobby, store, consent
remain blocked, while **Settings** ([D75](../decisions.md)) and **match setup** (the desktop
skirmish-setup screen, [`modes.md`](../modes.md) §3) are PARTIAL with desktop halves landed. See
[`roadmap.md`](../roadmap.md) §"Phase 4 — Polish & ship",
