...
| 4 | **Match setup** | Army/loadout composition, map + mode select; skirmish-vs-PvP entry | order/stance vocab (D25); **[Q5](../open-questions.md)** | **PARTIAL — the desktop skirmish half has landed** ([`modes.md`](../modes.md) §3 / build-order step 1): the egui skirmish-setup screen behind the title's SKIRMISH door (battlefield / both armies / opponent tier, wired through `apply_campaign_tuning` + `select_army` for both sides — zero engine work). Still owed: the Android Compose twin ([`compose-shell-parity.md`](compose-shell-parity.md) §12) and the map-library manifest picker. The **PvP half stays BLOCKED — [Q5](../open-questions.md)** *and* Phase 3 net (modes.md §5 steps 2–4); PvP's title door keeps the lightweight mode picker until then. |
