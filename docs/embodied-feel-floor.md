...
- [ ] **Audio is coupled to the visual.** Fire/impact cues land in lockstep with the muzzle/impact
  ...
  ([D100](decisions.md)) — designed Csound+SoX cues served by `pal::bank`; WS-A owns only the
  **coupling timing**.)
...
New host-clock cues (designed CP-6 assets since [D100](decisions.md), served by `pal::bank`):
