...
| **SoX** | audio | SFX synthesis + processing — the shared master chain (`-D`, no dither, for byte-determinism) every CP-6 cue passes through (`tools/audio/gen_sfx.py`, D100) |
| **Csound** | audio | deterministic, **seed-scripted** SFX — regenerable + git-diffable, the audio analogue of D41 (audio is a primary system, invariant #6); **drives the shipped CP-6 cue set** (`tools/audio/gen_sfx.py` → `assets/audio/`, decoded at stream-open by `pal::bank`, D100) |
